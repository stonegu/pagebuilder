...
public class PageController {
   ...
   @DeleteMapping("/delete/{pageId}/{uuid}")
   public ResponseEntity<PbApiResponse<PageWithoutBodyResponse>> deletePageById(
      @PathVariable("pageId") Long pageId,
      @PathVariable("uuid") String uuid
   ) {
      PageWithoutBodyResponse thePage = pageService.deletePage(pageId, uuid);
      PbApiResponse<PageWithoutBodyResponse> apiResponse = ApiResponseUtils.createApiResponse(thePage, logIdGenerator.getLogId());
      return new ResponseEntity<>(apiResponse, HttpStatus.OK);
   }
